...
<script>
    ...
    export default {
        ...
        methods: {
            getPageBlogs: function (current) {     // 页码改变传入新的页码，此处做回调
                var that = this;
                current = current - 1;
                this.blogs = EventHub.blogs.filter(function (v, index) {
                    if (index >= current * that.pageInfo.pagenum && index < current * that.pageInfo.pagenum + that.pageInfo.pagenum)
                        return true;
                });
            },
            navSearchKey(searchKey){
                this.blogs = EventHub.blogs.filter(function (v) {
                    if (v.title.includes(searchKey) ||
