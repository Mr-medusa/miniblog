<template>
    <div class="read-blog">
        ...
        <div class="read-btn-group">
            <a class="hvr-wobble-top" href="javascript:;" @click="editBlog">修改</a>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            lastPage(){
                var myPos = EventHub.blogs.indexOf(this.readOrEdit.blog);
                if(myPos-1 >= 0){
                    EventHub.pageInfo.current = Math.ceil((myPos-2) / EventHub.pageInfo.pagenum);
                    this.$router.push({ name: 'blogArticle', params: { id: EventHub.blogs[myPos-1].id, isReadable: true }});
                }else{
            ...
            nextPage(){
                var myPos = EventHub.blogs.indexOf(this.readOrEdit.blog);
                if(myPos+1 < EventHub.blogs.length){
                    EventHub.pageInfo.current = Math.ceil((myPos+2) / EventHub.pageInfo.pagenum);
                    this.$router.push({ name: 'blogArticle', params: { id: EventHub.blogs[myPos+1].id, isReadable: true }});
                }else{
