<template>
    <div class="edit_blog">
        ...
        <div class="title"><input placeholder="输入标题.." type="text" v-model="readOrEdit.blog.title"></div>
        <div class="more-info">
            <div class="author word-ellipsis" id="blog_author_input" @click="isEditAuthor=true">
                <i class="fa fa-user-o" aria-hidden="true"></i>
                <span v-show="!isEditAuthor">{{readOrEdit.blog.author}}</span>
                <input v-show="isEditAuthor || editableAuthor" type="text" v-model="readOrEdit.blog.author" placeholder="作者名称...">
            </div>
            <div class="date">
            ...
            <div class="tags" id="blog_tags_input" @click="isEditTags=true">
                ...
                <input v-show="isEditTags || editableTags" type="text" v-model="tags" placeholder="标签..(多个标签以空格分割)">
            </div>

        </div>
        <div class="content">
            <div id="blog-editor" @click="clickBlogEditor" v-html="myHtmlContent"></div>
        </div>
        <div class="commit">
    ...
    export default {
        ...
        mounted(){
            ...
            this.$nextTick(function(){
                if($('#blog-editor')[0])
                    this.blogEditor = Utils.makeQuillEditor("#blog-editor",this.myEidtorBinds());
            });
        ...
        methods:{
            ...
            saveBlog(){
                ...
                this.readOrEdit.blog.previewContent = this.blogEditor.getText().substr(0,150);
